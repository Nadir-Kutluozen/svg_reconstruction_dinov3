...
if __name__ == "__main__":
    ...
    parser.add_argument("--faces", type=int, choices=[1, 2], default=1, help="Which dataset to process")
    parser.add_argument("--png_dir", type=str, default=None, help="Override png directory path")
    parser.add_argument("--out_dir", type=str, default=DATA_DIR, help="Where to save the npz files")
    args = parser.parse_args()
    
    os.makedirs(args.out_dir, exist_ok=True)
    
    if args.png_dir is None:
        dataset_name = "svg_face_dataset_one_face" if args.faces == 1 else "svg_face_dataset_two_faces"
        # Check backward compatibility path
        if not os.path.exists(os.path.join(args.out_dir, dataset_name, "pngs")):
             # Try old path
             old_name = "svg_face_dataset_one_face" if args.faces == 1 else "svg_face_dataset_two_face"
             png_dir = os.path.join(old_name, "pngs")
        else:
    ...
    suffix = "" if args.faces == 1 else "_twofaces"
    ...
    # Pretrained Model
    print(f"\n--- Extracting Pretrained DINOv3 Features for {args.faces}-Face dataset ---")
    ...
    # Random Init Model
    print(f"\n--- Extracting Random DINOv3 Features for {args.faces}-Face dataset ---")
